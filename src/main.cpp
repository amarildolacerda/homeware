...
void setup()
{
  ...
  homeware.debugf("Portal starting...'");
  portal.setup(&server);
  homeware.debugf("AutoConnect starting...'");
  portal.autoConnect(homeware.config["label"]);
  Serial.printf("Ver: %s \r\n", VERSION);

  homeware.debugf("setupServer starting...'");
  // setupServer();
  defaultConfig();
  ...
  // chear se D2 esta pressionado ao iniciar
  int pinValue = digitalRead(4);
  if (pinValue == HIGH)
  {
    Serial.println("D2 pressionado, resetando...");
    WiFiManager wf;
    wf.resetSettings();
    delay(5000);
    ESP.restart();
