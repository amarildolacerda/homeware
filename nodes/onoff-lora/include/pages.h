...
<title id="pageTitle">LoRa Switch</title>
<style>
...
input[type=password]{padding:6px 10px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:.82rem;outline:none;width:100%}
...
<div class="dev-name" id="sbName">LoRa Switch</div>
...
<div style="margin-top:6px"><span class="badge off" id="relayBadge">OFF</span></div>
</div>
<div class="card">
...
<div class="row"><span class="label">Estado</span><span class="value" id="detState">—</span></div>
<div class="row"><span class="label">Pareado</span><span class="value" id="detPaired">—</span></div>
...
<div class="row"><span class="label">WiFi</span><span class="value" id="detWifi">—</span></div>
</div>
</div>
...
<div class="row"><span class="label">Gateway</span><span class="value" id="propGateway">—</span></div>
<div class="row"><span class="label">WiFi</span><span class="value" id="propWifi">—</span></div>
</div>
</div>
...
<span class="label" style="display:block;margin-bottom:4px">Rede WiFi</span>
<span class="value" id="wSsid" style="display:block;margin-bottom:6px">—</span>
<div style="display:flex;gap:6px">
<input type="text" id="wNewSsid" placeholder="SSID" style="flex:1">
<input type="password" id="wNewPass" placeholder="Senha" style="flex:1">
<button class="btn btn-primary btn-sm" onclick="saveWifi()">Conectar</button>
</div>
<div id="wMsg" style="font-size:.72rem;margin-top:4px;color:var(--muted-subtle)"></div>
</div>
</div>
...
let loading=false,currentSection='home';
...
async function saveWifi(){let ssid=document.getElementById('wNewSsid').value.trim();if(!ssid)return;let pass=document.getElementById('wNewPass').value;let msg=document.getElementById('wMsg');msg.textContent='Conectando...';try{let r=await fetch('/api/wifi',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ssid:ssid,password:pass})});let d=await r.json();msg.textContent=d.status==='ok'?'Conectando à rede...':'Erro';}catch(e){msg.textContent='Erro: '+e.message}}
async function fetchState(){try{let r=await fetch('/api/state');let d=await r.json();
const on=d.relay;btn.classList.toggle('on',on);badge.textContent=on?'ON':'OFF';badge.className='badge '+(on?'on':'off');
rxVal.textContent=d.rx_count||0;txVal.textContent=d.tx_count||0;memVal.textContent=d.free_heap||0;
document.getElementById('detState').textContent=on?'ON':'OFF';
document.getElementById('detPaired').textContent=d.paired?'Sim':'Não';
...
document.getElementById('detWifi').textContent=(d.wifi_ssid||'--')+' ch'+(d.wifi_channel||'-');
document.getElementById('wSsid').textContent=(d.wifi_ssid||'—')+(d.wifi_ssid?' ['+d.ip+']':'');
...
document.getElementById('propGateway').textContent=d.gateway_connected?'Online':'Offline';
document.getElementById('propGateway').className='value'+(d.gateway_connected?' green':'');
document.getElementById('propWifi').textContent=(d.wifi_ssid||'--')+' ch'+(d.wifi_channel||'-');
let name=d.device_name||'LoRa Switch';
...
footerEl.textContent=d.device_id+(d.last_send_s?' Último envio: '+d.last_send_s+'s':'');
...
try{let r=await fetch('/api/relay',{method:'POST'});let d=await r.json();const on=d.relay;btn.classList.toggle('on',on);badge.textContent=on?'ON':'OFF';badge.className='badge '+(on?'on':'off');
...
<div class="endpoint"><div class="head"><span class="method post">POST</span><span class="path">/api/relay</span></div><div class="desc">Alterna relé</div></div>
...
<script>
async function loadStatus(){try{let r=await fetch('/api/wifi');let d=await r.json();document.getElementById('ssid').value=d.ssid||'';document.getElementById('password').value=d.password||'';document.getElementById('devName').value=d.device_name||''}catch(e){}}
...
if(d.status==='ok'){showMsg('Conectando...','ok')}else{showMsg('Erro: '+d.status,'err');loading(false)}}
