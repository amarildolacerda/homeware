...
static uint8_t s_motion_state = 0;
static unsigned long s_motion_start_time = 0;
...
static void read_pir(void)
{
    ...
    if (pir_val == HIGH && !s_motion_state)
    {
        s_motion_state = 1;
        s_motion_start_time = millis();
    }
    else if (s_motion_state && (millis() - s_motion_start_time > PIR_HOLD_TIME_MS))
    {
        s_motion_state = 0;
    }
    ...
    console.printf("Sensor: motion=%d bat=%d%%\n", s_motion_state, s_battery);
    s_read_valid = true;
}
...
void setup(void)
{
    ...
    console.printf("\n============================================\n");
    console.printf("  AgriSense Presence Bat " FW_VERSION "\n");
    ...
    bool button_pressed = (digitalRead(CFG_BUTTON_PIN) == LOW);
