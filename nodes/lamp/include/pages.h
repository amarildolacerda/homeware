...
async function fetchWifi(){try{let r=await fetch('/api/wifi');let d=await r.json();let st=document.getElementById('wifiStatus');if(st){st.textContent=d.status==='connected'?'Conectado ('+d.ssid+')':'Desconectado';st.className='value'+(d.status==='connected'?' green':'')}if(d.ssid)document.getElementById('wifiSsid').value=d.ssid;if(d.channel!==undefined)document.getElementById('wifiChannel').value=d.channel}catch(e){}}
