...
let pass=document.getElementById('password').value;let name=document.getElementById('devName').value.trim();let ch=parseInt(document.getElementById('wifiChannel').value)||0;loading(true);
let body={ssid:ssid,password:pass};if(name)body.device_name=name;if(ch>=0&&ch<=13)body.channel=ch;
