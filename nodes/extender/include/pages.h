...
<title id="pageTitle">Repeater</title>
<style>
:root{--bg:#f4f4f4;--surface:#fff;--surface-2:#f9fafb;--text:#1f2937;--muted:#6b7280;--muted-subtle:#9ca3af;--primary:#5e6ad2;--primary-strong:#828fff;--primary-focus:#eef0ff;--border:#e5e7eb;--border-strong:#d1d5db;--success:#16a34a;--danger:#dc2626;--warn:#f59e0b;--info:#2563eb;--sidebar-w:180px}
...
.hero-card{background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center}
.hero-card .hc-value{font-size:1.6rem;font-weight:700;color:var(--primary)}
.hero-card .hc-label{font-size:.68rem;color:var(--muted-subtle);text-transform:uppercase;letter-spacing:.03em;margin-top:4px}
.card{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:16px;margin-top:16px}
.card-title{font-size:.85rem;font-weight:600;color:var(--primary);margin-bottom:8px}
...
th{text-align:left;color:var(--muted-subtle);padding:6px 4px;border-bottom:1px solid var(--border);font-size:.72rem;text-transform:uppercase;letter-spacing:.03em}
td{padding:6px 4px;border-bottom:1px solid var(--border);font-family:monospace;font-size:.78rem}
...
@media(max-width:600px){.sidebar{width:48px}.sidebar-top .dev-name,.sidebar-top .dev-id,.sidebar-bottom{display:none}.nav-item{justify-content:center;padding:10px 4px}.nav-item span{display:none}.nav-item span:first-child{display:inline;font-size:1.1rem}.main{margin-left:48px}.stats-header{padding:5px 10px}.content{padding:12px}.footer-bar{padding:6px 12px;font-size:.7rem;gap:8px}.hero{grid-template-columns:1fr 1fr;gap:8px}}
</style>
</head>
<body>
<div class="sidebar">
<div class="sidebar-top">
<div class="dev-name" id="sbName">AgriSense Extender</div>
...
<div class="stat"><div class="stat-value" id="clientsVal">0</div><div class="stat-label">Clients</div></div>
</div>
<div class="content">
<div class="section active" id="secHome">
<div class="hero">
<div class="hero-card"><div class="hc-value" id="hRec">0</div><div class="hc-label">Recebidos</div></div>
<div class="hero-card"><div class="hc-value" id="hFwd">0</div><div class="hc-label">Encaminhados</div></div>
<div class="hero-card"><div class="hc-value" id="hAct">0</div><div class="hc-label">Clientes Ativos</div></div>
<div class="hero-card"><div class="hc-value" id="hLast">-</div><div class="hc-label">&#218;ltima Atividade</div></div>
</div>
<div class="card">
...
var clientsVal=document.getElementById('clientsVal');
...
clientsVal.textContent=d.clients||0;
document.getElementById('hRec').textContent=d.received||0;
document.getElementById('hFwd').textContent=d.forwarded||0;
...
var name=d.device_name||'AgriSense Extender';
...
fbDot.className='fb-dot'+(gwConnected?' online':' offline');
fbGateway.textContent=gwConnected?'Online':'Offline';
...
<div class="endpoint"><div class="head"><span class="method get">GET</span><span class="path">/api/status</span></div><div class="desc">Status do repeater (gateway, uptime, channel, RSSI, clients, contadores)</div></div>
<div class="endpoint"><div class="head"><span class="method get">GET</span><span class="path">/api/settings</span></div><div class="desc">Configura&#231;&#245;es do dispositivo</div></div>
<div class="endpoint"><div class="head"><span class="method post">POST</span><span class="path">/api/settings</span></div><div class="desc">Atualizar <code>{"device_name":"..."}</code></div></div>
...
<input type="text" id="devName" placeholder="Ex: Repeater">
<button type="submit" class="btn" id="submitBtn">Conectar</button>
</form>
<div id="msg" class="msg" style="display:none"></div>
</div>
<script>
async function loadStatus(){try{var r=await fetch('/api/wifi');var d=await r.json();document.getElementById('ssid').value=d.ssid||'';document.getElementById('devName').value=d.device_name||''}catch(e){}}
