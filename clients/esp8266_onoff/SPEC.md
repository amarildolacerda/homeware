# Spec: ESP8266 OnOff (ON/OFF Relay)

## Visão Geral

Cliente ESP8266 (Wemos D1 Mini) que controla um relé ON/OFF. Comunica com o Gateway via ESP-NOW, suporta Alexa (Espalexa/Hue Bridge), timer programável e atua como repeater ESP-NOW bidirecional.
...
- **Dados**: envia `ESPNOW_MSG_SENSOR_DATA` com `payload_onoff_t` (state 0/1) + IP embutido + heap livre
...
- Nome: configurado via WiFiManager (padrão: `OnOff`)
...
- Até 6 timers configuráveis via API (`/api/timers`)
- Cada timer: horário (hora/minuto), ação (ON/OFF), máscara de dias (bit0=Dom...bit6=Sáb, 0=todos)
- Persistência em EEPROM
- `timer_check()` chamado no loop a cada `TIMER_CHECK_INTERVAL_MS` (10s)
- Retorna -1 (sem ação), 0 (OFF), 1 (ON)
- Callback `on_timer_fire(action)` no main.cpp executa a ação
