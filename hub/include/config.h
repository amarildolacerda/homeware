#ifndef CONFIG_H
#define CONFIG_H

#include "platform.h"
#include "shared_config.h"

#define DEVICE_NAME PLATFORM_PREFIX "_Gateway"

#define MAX_VIRTUAL_SENSORS 64
#define PAIRING_WINDOW_MS 60000
#define HEARTBEAT_INTERVAL_MS 30000
#define SENSOR_TIMEOUT_MS 300000
#define PAIR_BUTTON_GPIO 0
#define STATUS_LED_GPIO 2
#define LOG_BUFFER_SIZE 50
#define LOG_MSG_MAX 64

#define TIME_SYNC_INTERVAL_MS 300000
#define NTP_RETRY_INTERVAL_MS 1800000
#define HUB_RX_WATCHDOG_MS 300000  // 5 min without any RX → restart
#define HUB_RX_HEALTHY_MS 60000    // janela de RX saudavel (2x heartbeat 30s)

#ifndef MQTT_HOST_DEFAULT
#define MQTT_HOST_DEFAULT "0.0.0.0"
#endif
#ifndef MQTT_PORT_DEFAULT
#define MQTT_PORT_DEFAULT 1883
#endif
#ifndef MQTT_USER_DEFAULT
#define MQTT_USER_DEFAULT ""
#endif
#ifndef MQTT_PASS_DEFAULT
#define MQTT_PASS_DEFAULT ""
#endif
#define EEPROM_SENSOR_BASE 0
#define EEPROM_SENSOR_SIZE 58
#define EEPROM_SENSOR_BRIDGE_ID_OFFSET 43
#define EEPROM_SENSOR_BRIDGE_ID_SIZE 15
#define EEPROM_SENSOR_MAX_SLOTS MAX_VIRTUAL_SENSORS
#define EEPROM_MQTT_HOST_OFFSET (EEPROM_SENSOR_BASE + EEPROM_SENSOR_MAX_SLOTS * EEPROM_SENSOR_SIZE)
#define EEPROM_MQTT_PORT_OFFSET (EEPROM_MQTT_HOST_OFFSET + 64)
#define EEPROM_MQTT_USER_OFFSET (EEPROM_MQTT_PORT_OFFSET + 2)
#define EEPROM_MQTT_PASS_OFFSET (EEPROM_MQTT_USER_OFFSET + 32)

#define EEPROM_WIFI_SSID_OFFSET (EEPROM_MQTT_PASS_OFFSET + 32)
#define EEPROM_WIFI_SSID_SIZE 33
#define EEPROM_WIFI_PASS_OFFSET (EEPROM_WIFI_SSID_OFFSET + EEPROM_WIFI_SSID_SIZE)
#define EEPROM_WIFI_PASS_SIZE 65

#define EEPROM_WIFI_MODE_OFFSET (EEPROM_WIFI_PASS_OFFSET + EEPROM_WIFI_PASS_SIZE)
#define EEPROM_WIFI_MODE_SIZE 1
#define EEPROM_WIFI_IP_OFFSET (EEPROM_WIFI_MODE_OFFSET + EEPROM_WIFI_MODE_SIZE)
#define EEPROM_WIFI_IP_SIZE 16
#define EEPROM_WIFI_GW_OFFSET (EEPROM_WIFI_IP_OFFSET + EEPROM_WIFI_IP_SIZE)
#define EEPROM_WIFI_GW_SIZE 16
#define EEPROM_WIFI_MASK_OFFSET (EEPROM_WIFI_GW_OFFSET + EEPROM_WIFI_GW_SIZE)
#define EEPROM_WIFI_MASK_SIZE 16
#define EEPROM_WIFI_DNS_OFFSET (EEPROM_WIFI_MASK_OFFSET + EEPROM_WIFI_MASK_SIZE)
#define EEPROM_WIFI_DNS_SIZE 16

#define EEPROM_PAIRING_EN_OFFSET (EEPROM_WIFI_DNS_OFFSET + EEPROM_WIFI_DNS_SIZE)
#define EEPROM_OP_MODE_OFFSET (EEPROM_PAIRING_EN_OFFSET + 1)

// Telegram config
#define EEPROM_TELEGRAM_EN_OFFSET (EEPROM_OP_MODE_OFFSET + 1)
#define EEPROM_TELEGRAM_TOKEN_OFFSET (EEPROM_TELEGRAM_EN_OFFSET + 1)
#define EEPROM_TELEGRAM_TOKEN_SIZE 64
#define EEPROM_TELEGRAM_CHATID_OFFSET (EEPROM_TELEGRAM_TOKEN_OFFSET + EEPROM_TELEGRAM_TOKEN_SIZE)
#define EEPROM_TELEGRAM_CHATID_SIZE 20
#define EEPROM_TELEGRAM_POLL_OFFSET (EEPROM_TELEGRAM_CHATID_OFFSET + EEPROM_TELEGRAM_CHATID_SIZE)
#define EEPROM_TELEGRAM_POLL_SIZE 4  // uint32_t
#define EEPROM_TELEGRAM_ALERTS_LVL_OFFSET (EEPROM_TELEGRAM_POLL_OFFSET + EEPROM_TELEGRAM_POLL_SIZE)
#define EEPROM_TELEGRAM_ALERTS_LVL_SIZE 1  // bitmask: bit0=critical, bit1=alert, bit2=warning, bit3=info
#define EEPROM_TELEGRAM_ALERTS_TYPE_OFFSET (EEPROM_TELEGRAM_ALERTS_LVL_OFFSET + EEPROM_TELEGRAM_ALERTS_LVL_SIZE)
#define EEPROM_TELEGRAM_ALERTS_TYPE_SIZE 2  // bitmask: bit0=gas, bit1=smoke, bit2=offline, bit3=reconnect, bit4=battery, bit5=temp, bit6=humidity, bit7=rssi, bit8=heap, bit9=daily_report

#define EEPROM_SIZE (EEPROM_TELEGRAM_ALERTS_TYPE_OFFSET + EEPROM_TELEGRAM_ALERTS_TYPE_SIZE)

#define WIFI_CONFIG_PORTAL_SSID "AgroHub_Setup"
#define WIFI_CONFIG_PORTAL_PASS "password123"

// Operation modes
#define OP_MODE_TERMINAL 0
#define OP_MODE_AP       1
#define OP_MODE_HYBRID   2

// Operational AP config (modes 1 and 2)
#define AP_CHANNEL 1
#define AP_PASS    "password123"

#endif