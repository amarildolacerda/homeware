...
18. **ESP-NOW broadcast vs unicast (ESP32 ↔ ESP8266)**: Quando gateway (ESP32 STA) e clientes (ESP8266 STA) estão no MESMO AP, unicast ESP-NOW pode falhar por coexistência rádio (comprovado em teste isolado). Estratégia adotada: **unicast como primário, com fallback para broadcast** quando `esp_now_send` retorna erro. Broadcast ainda é usado para PAIR_REQUEST (descoberta) e TIME_SYNC (todos os clients). O receptor identifica o destinatário pelos campos `sensor_mac`/`target_mac`.
