...
51. **ESPAsyncWebServer e corpo JSON (POST)**: após migrar o hub para `AsyncWebServer` (ESP32, `ASYNC_WEB_ENABLED`), corpos de POST JSON NÃO chegam via `request->arg("body")`/`hasParam("body", true)` — o `ESPAsyncWebServer` só roteia `application/json` para um body handler. Usar `AsyncCallbackJsonWebHandler` (incluir `#include "AsyncJson.h"`) e ler de `JsonVariant json` (`deserializeJson(doc, json.as<String>())`). O dashboard (`pages.h`, helper `api()`) e os nodes TCP (`shared/src/tcp_node_protocol.cpp`) já enviam `Content-Type: application/json`, então o handler dispara. Sintoma ao errar: todo POST JSON retorna 400 e os devices ficam `offline` no dashboard (last_seen nunca atualiza). Afeta `/node/state`, `/node/heartbeat`, `/node/register` (tcp_radio_handler.cpp) e os POSTs de config (web_server.cpp). Regra aprendida 2026-08-26.
...
- `hub` — v1.2.22 (2026-08-28) — ESP-NOW + TCP Hub, WiFi 3-tier (EEPROM→STATIC_WIFI→AP), NTP+time sync broadcast, MQTT discovery
