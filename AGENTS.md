...
24. **Clientes bateria (`_bat`)**: clients com deep sleep usam sufixo `_bat` no nome da pasta (`esp8266_soil_moisture_bat`, `esp8266_pir_bat`). Seguem o modelo: deep sleep entre ciclos, telnet suspende sleep (via `console.telnet_connected()`), telnet disconnect faz restart, comando `p` re-pareia, restart via `ESPNOW_MSG_RESTART` do gateway, menu serial/telnet com `s/i/r/p/h`, intervalo configurável via WiFiManager.
